    ...
    public function getProducts($refresh = false, $id_product = false, $id_country = null) {
        ...
        foreach ($result as &$row) {
            ...
            $row['price'] = Product::getPriceStatic(
                            (int) $row['id_product'], false, isset($row['id_product_attribute']) ? (int) $row['id_product_attribute'] : null, 6, null, false, true, $row['cart_quantity'], false, (int) $this->id_customer ? (int) $this->id_customer : null, (int) $this->id, $address_id, $specific_price_output, false, true, $cart_shop_context
            );
    ...
    public function getOrderTotal($with_taxes = true, $type = Cart::BOTH, $products = null, $id_carrier = null, $use_cache = true) {
        ...
        foreach ($products as $product) { // products refer to the cart details
            ...
            $price = Product::getPriceStatic(
                            (int) $product['id_product'], false, (int) $product['id_product_attribute'], 6, null, false, true, $product['cart_quantity'], false, (int) $this->id_customer ? (int) $this->id_customer : null, (int) $this->id, $id_address, $null, false, true, $virtual_context
            );
     ...
     /**
     * This function returns the total cart amount convert to XOF
     *
     *
     * @param boolean $withTaxes With or without taxes
     * @param integer $type Total type
     * @param integer $currency_id  Current currency id
     * @return float Order total convert form current currency to XOF
     */

    public function getConvertOrderTotal($with_taxes = true, $type = Cart::BOTH, $currency_id) {
        $amount = 0;
        $amount = $this->getOrderTotal(true, Cart::BOTH);
        $currency_from = new Currency((int) $currency_id);
        return Tools::convertPriceFull($amount, $currency_from, null);
    }
